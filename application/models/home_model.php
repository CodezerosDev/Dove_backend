...
class Home_model extends BF_Model {
    ...
    public function fetch_pending_app_user()
    {

        $query = $this->db->select('count(id) as totaluser',false)->from('bf_users')->where('role_id',4)->where('is_verified',0)->get()->first_row();
    ...
    public function fetch_verified_app_user()
    {

        $query = $this->db->select('count(id) as totaluser',false)->from('bf_users')->where('role_id',4)->where('is_verified',1)->get()->first_row();
    ...
    public function fetch_rejected_app_user()
    {

        $query = $this->db->select('count(id) as totaluser',false)->from('bf_users')->where('role_id',4)->where('is_verified',2)->get()->first_row();
