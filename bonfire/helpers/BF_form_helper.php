...
<div class='control-group{$error}'>
    <label class='control-label col-md-3 ' for='{$defaults['id']}'>{$label}</label>
    <div class='controls'>
         <input {$output} {$extra} />
        {$tooltip}
    </div>
    ...
    function form_dropdown($data, $options = array(), $selected = array(), $label = '', $extra = '', $tooltip = '')
    {
        ...
        foreach ($options as $key => $val) {
            ...
            if (is_array($val) && ! empty($val)) {
                $options_vals .= "<optgroup class='form-control input-medium' label='{$key}'>" . PHP_EOL;
        ...
        return "
<div class='control-group{$error}'>
    <label class='control-label col-md-3' for='{$data['id']}'>{$label}</label>
    <div class='controls'>
         <select {$output} {$extra}>
            {$options_vals}
        </select>
