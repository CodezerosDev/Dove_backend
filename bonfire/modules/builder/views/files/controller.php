...
if ($this->input->post('use_position') == 'true') {
    ...
    $position_part .= '$this->db->select_max("' . $tmp_position_field . '");'.PHP_EOL;
    $position_part .= '$query = $this->db->get("' . $table_name . '");'.PHP_EOL;
    $position_part .= '$pos = array_shift($query->result());'.PHP_EOL;
    $position_part .= '$data["' . $tmp_position_field . '"] = $pos->' . $tmp_position_field . ' + 1;';
}

if ($this->input->post('use_status') == 'true') {
    $tmp_status_field = $this->input->post("status_field");
    $status_rule .= '
		$this->form_validation->set_rules(\'' . $tmp_status_field . '\',\'Status\', \'required\');';
    $status_data .= "\n\t\t" . '$data[\'' . $tmp_status_field . '\']        = $this->input->post(\'' . $tmp_status_field . '\');';
}
/*Grid Area Ends */
...
if ($db_required != '') {
    ...
    // Admin controllers
        $indexDelete .=<<<EOT
        \$result = \$this->{$module_name_lower}_model->read(\$this->input->post());
...
EOT;
    ...
    if ($this->input->post('use_position') == 'true') {
        $indexFind .= <<<EOT
            \$maxPosition = \$this->{$module_name_lower}_model->get_max_position(\$this->input->post('category'));
            \$minPosition = \$this->{$module_name_lower}_model->get_min_position(\$this->input->post('category'));
