...
class Users extends Front_Controller
{
	...
	public function __construct()
	{
        ...
        $this->load->model('my_tokens/my_tokens_model');
	...
	public function logout()
	{
		...
		$this->auth->logout();
        /*Template::redirect('/');*/
        Template::redirect('http://dove.network');
    ...
    public function profile()
    {
        ...
        if (isset($_POST['save'])) {
            ...
            if(!empty($_POST['proff_doc']['name']))
            {

                $userimage = $this->userkycUpload();

                if (!empty($userimage)) {
                    if (!empty($userimage['file_name'])) {
                        $_POST['proff_doc'] = $userimage['file_name'];
                        //$query = $this->db->get_where('user_master', array('id' => $id));

                        //$result = array_shift($query->result());

                        if(isset($_POST['proff_doc_old']) && trim($_POST['proff_doc_old'])!='' && !is_null($_POST['proff_doc_old']))
                        {
                            $originalPath = FCPATH . "assets/uploads/kyc/original/" . $_POST['proff_doc_old'];
                            $resizePath = FCPATH . "assets/uploads/kyc/" . $_POST['proff_doc_old'];
    ...
    public function kyc()
    {
        ...
        if (isset($_POST['save'])) {
            ...
            if(!empty($_POST['proff_id_doc']['name']))
            {

                $userimage = $this->userkycIdUpload();

                if (!empty($userimage)) {
                    if (!empty($userimage['file_name'])) {
                        $_POST['proff_id_doc'] = $userimage['file_name'];

                        if(isset($_POST['proff_id_doc_old']) && trim($_POST['proff_id_doc_old'])!='' && !is_null($_POST['proff_id_doc_old']))
                        {
                            $originalPath = FCPATH . "assets/uploads/kyc/original/" . $_POST['proff_id_doc_old'];
                            $resizePath = FCPATH . "assets/uploads/kyc/" . $_POST['proff_id_doc_old'];
                            array_map("unlink", glob($originalPath));
                            array_map("unlink", glob($resizePath));
                        }
                    }
            ...
            if ($this->saveUser('update', $user_id, $meta_fields)) {
                ...
                log_activity(
                    $this->current_user->id,
                    lang('us_log_kyc_profile') . ": {$log_name}",
                    'users'
                );

                Template::set_message(lang('us_kyc_updated_success'), 'success');

                // Redirect to make sure any language changes are picked up.
                Template::redirect('/users/kyc');
            }

            Template::set_message(lang('us_kyc_updated_error'), 'error');
        }
        ...
        Template::set_view('kyc');
        Template::render();
    }

    public function my_buy_token()
    {
        ...
        $result = $this->my_tokens_model->read($this->input->post());
        ...
        $records = $this->my_tokens_model->find_by('user_id',$this->current_user->id);
        ...
        Template::set_view('my_buy_token');
        Template::render();
    }

    public function userkycUpload()
    {

        $config['upload_path'] = FCPATH . 'assets/uploads/kyc/original';//file upload path
        $config['allowed_types'] = 'gif|jpg|png|jpeg|pdf|doc|docx';//file type allowed
        $config['file_name'] = $_POST['proff_doc']['name'];
        //not overwrite image for below code
        $config['overwrite'] = FALSE;
        $config['max_size'] = '5120';//max file size for upload
        ...
        if (!empty($_POST['proff_doc']['name']) && $_POST['proff_doc']['error'] != 4) {

            if (!$this->upload->do_upload('proff_doc')) {
            ...
            else
            {
                ...
                $config['new_image'] = FCPATH . 'assets/uploads/kyc/' . $upload_data['file_name'];
                $config['quality'] = "100%";
    ...
    public function userkycIdUpload()
    {

        $config['upload_path'] = FCPATH . 'assets/uploads/kyc/original';//file upload path
        $config['allowed_types'] = 'gif|jpg|png|jpeg|kyc|doc|docx';//file type allowed
        $config['file_name'] = $_POST['proff_id_doc']['name'];
        //not overwrite image for below code
        $config['overwrite'] = FALSE;
        $config['max_size'] = '5120';//max file size for upload
        ...
        if (!empty($_POST['proff_id_doc']['name']) && $_POST['proff_id_doc']['error'] != 4) {

            if (!$this->upload->do_upload('proff_id_doc')) {
    ...
    private function saveUser($type = 'insert', $id = 0, $metaFields = array())
	{
        ...
        if(isset($_POST['from']) && $_POST['from'] != 'kyc'){
        ...
        if ($type != 'insert' && $this->input->post('password')) {
            $this->form_validation->set_rules('pass_confirm', 'lang:bf_password_confirm', "required");
        }
        ...
        if ($type != 'insert' && isset($_POST['from']) && $_POST['from'] != 'profile') {

            foreach ($metaFields as $field) {
                $adminOnlyField = isset($field['admin_only']) && $field['admin_only'] === true;
                $frontEndField = !isset($field['frontend']) || $field['frontend'];
                if ($frontEndField && ($userIsAdmin || !$adminOnlyField)) {
                    $this->form_validation->set_rules($field['name'], $field['label'], $field['rules']);

                    if($field['name']=='proff_doc' && (isset($_POST['proff_doc']['name']) && $_POST['proff_doc']['name']=='')){

                    }else{
                        if($field['name']=='proff_id_doc' && (isset($_POST['proff_id_doc']['name']) && $_POST['proff_id_doc']['name']=='')){
        ...
        if($this->input->post('username')){
            $_POST['display_name'] = $this->input->post('username');
        }
        ...
        if ($type == 'insert') {
            ...
            if(isset($_POST['referral_code'])){
                $data['referral_code'] = trim($this->input->post('referral_code'));
            }
