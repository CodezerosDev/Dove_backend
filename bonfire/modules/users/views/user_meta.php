...
if ( ! empty($meta_fields)) :
    ...
    foreach ($meta_fields as $field) :
        ...
        if ($field['form_detail']['type'] == 'dropdown') :
            echo form_dropdown($field['form_detail']['settings'], $field['form_detail']['options'], set_value($field['name'], isset($user->$field['name']) ? $user->$field['name'] : ''), $field['label'],"class=>'form-control input-medium'");

        elseif ($field['form_detail']['type'] == 'checkbox') :
                ?>
                <div class="control-group <?php echo iif( form_error($field['name']) , 'has-error'); ?>">
        ...
        elseif ($field['form_detail']['type'] == 'state_select' && is_callable('state_select')) :
            ...
            ?>
            <div class="control-group <?php echo iif( form_error($field['name']) , 'has-error'); ?>">
                <label class="control-label col-md-3" for="<?php echo $field['name'] ?>"><?php echo lang('user_meta_state'); ?></label>
                <div class="controls">
                    <?php echo state_select(set_value($field['name'], $stateValue), $defaultState, $defaultCountry, $field['name'], 'chzn-select form-control input-medium'); ?>
                </div>
            </div>
            <?php
        elseif ($field['form_detail']['type'] == 'country_select' && is_callable('country_select')) :
            ...
            $countryValue = isset($user->country) ? $user->country : $defaultCountry;
            

            $typesclass = ($this->session->userdata('role_id') == 4)?'chzn-select form-control input-medium span6':'chzn-select form-control input-medium';
            ?>
            <div class="control-group <?php echo iif( form_error('country') , 'has-error'); ?>">
                <label class="control-label col-md-3" for="country"><?php echo lang('user_meta_country'); ?></label>
                <div class="controls">
                    <?php echo country_select(set_value($field['name'], isset($user->country) ? $user->country : $defaultCountry), $defaultCountry, 'country', $typesclass); ?>
                </div>
            </div>
            <?php
        else:
            ...
            if (is_callable($form_method)) {
                ...
                $fname = $field['name'];
                echo $form_method($field['form_detail']['settings'], set_value($field['name'], isset($user->$fname) ? $user->$fname : ''), $field['label'],$this->session->userdata('role_id'));
            }
        endif;
    endforeach;
