<style>
    ...
    .user-table th.last-login {
        width: 11em;
    }

    .user-table th.status {
        width: 10em;
    }
</style>
    ...
    <li class="<?php echo $filter_type == 'role_id' ? 'active ' : ''; ?>dropdown">
        ...
        <ul class="dropdown-menu">
            <?php foreach ($roles as $role) : ?>
                ...
                if($this->session->userdata('role_id') == 8 && in_array($role->role_id,array(4,7))){?>
                <li><?php echo anchor("{$index_url}role_id-{$role->role_id}", $role->role_name); ?></li>
                <?php }else if($this->session->userdata('role_id') != 8){ ?>
                    <li><?php echo anchor("{$index_url}role_id-{$role->role_id}", $role->role_name); ?></li>
                <?php } ?>
            <?php endforeach; ?>
        </ul>
...
/*                echo lang('bf_with_selected');
                ...
                if ($filter_type == 'deleted') :
                    */?>
                    ...
                    <input type="submit" name="purge" class="btn btn-danger"
                ...
                <?php /*else : */?>
                    ...
                    <input type="submit" name="deactivate" class="btn"
                           value="<?php /*echo lang('bf_action_deactivate'); */?>"/>
        ...
        foreach ($users as $user) : ?>
            <?php if($this->session->userdata('role_id') == 8 && in_array($user->role_id,array(4,7))){?>
            <tr>
               <!-- <td class="column-check"><input type="checkbox" name="checked[]" value="<?php /*echo $user->id; */?>"/></td>-->
                ...
                <td><?php
                    ...
                    if ($user->banned) :
                        ?>
                        <span class="badge badge-warning"><?php echo lang('us_tab_banned'); ?></span>
                    <?php endif; ?>
                </td>
                ...
                <td class='status'>
                    <?php if ($user->active) : ?>
                        <span class="badge badge-success"><?php echo lang('us_active'); ?></span>
                    <?php else : ?>
                        <span class="badge badge-warning"><?php echo lang('us_inactive'); ?></span>
                    <?php endif; ?>
                </td>
            </tr>
        <?php }else if($this->session->userdata('role_id') != 8){ ?>
